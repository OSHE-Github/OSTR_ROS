...
def detect_frequency(data):
    ...
    filtered_data = lowpass_filter(data, cutoff=1200, fs=SAMPLE_RATE)
    filtered_data = highpass_filter(data, cutoff=200, fs=SAMPLE_RATE)
...
class AcousticReceiver(Node):
    def __init__(self):
        ...
        # Determine transmission side (Replace with actual hub flag logic)
        self.hub_flag = self.determine_hub_flag()

        try:
            if self.hub_flag == 1:  # Fish Hub side
                self.run_fishhub()
            elif self.hub_flag == 0:  # Hub side
                self.run_hub()
        except KeyboardInterrupt:
            self.get_logger().info("Receiver node shutting down.")
        finally:
            self.cleanup()

    def determine_hub_flag(self):
        # Placeholder: Replace with actual logic to get hub_flag from ROS2 system
        return 1  # Assume Fish Hub for now

    def run_fishhub(self):
        global PACKET_SIZE
    ...
    def run_hub(self):
        global PACKET_SIZE
        PACKET_SIZE = HUB_PACKET
        while rclpy.ok():
            # Receive size information
            size_command = receive_command(self.audio_stream, HUB_PACKET, self)
            listen_size = sum(2 ** (HUB_PACKET - 1 - i) for i, bit in enumerate(size_command) if bit == "1")

            # Receive data
            PACKET_SIZE = listen_size
            command = receive_command(self.audio_stream, PACKET_SIZE, self)
